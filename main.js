...
const menApi ="https://raw.githubusercontent.com/abdelaziz-belhachmi/Jsontesting/main/products/men/data";


const onLoad = async () =>{
  try {
    const dt= await fetch(menApi); 
    const dat = await dt.json();
    const data = dat.menproducts;
     console.log(data);
var cp =shuffle(data);
document.getElementById("skblk").innerHTML = ` ${ cp.map(generateCard).join('') } `;
  } catch (error) {
    console.log(error);
  }
...
const crd = document.getElementById("skblk").appendChild.innerHTML = ` <div class="pb-5 ">
...
searchBar.addEventListener('keyup',async (e)=> {
  ...
  // console.log(data);
  const searchStringa = e.target.value;
  const searchString =searchStringa.toUpperCase();
   const filtredChar = data.filter((character) => {
        return character.name.toUpperCase().includes(searchString);
  } );
  
  // console.log(filtredChar);
  document.getElementById("skblk").innerHTML =`${filtredChar.map(generateCard).join('')}`;
});
